...
\begin{itemize}
  \item \textbf{Customers}(\underline{customer\_id}, customer\_unique\_id,
        ...
        (279 record cliente, pari a 158 CAP distinti, senza corrispondenza nella tabella di geolocalizzazione; join eseguito come LEFT JOIN).}
...
\textbf{Risultato}: 19.010 record, 0 missing, 0 duplicati.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrl}
    ...
    rec\_geolocation  &  19.010 &  6 & Aggregata per ZIP \\
...
\begin{itemize}
  ...
  \item \textbf{Geolocalizzazione}: aggregando per ZIP si perde la variabilità
        ...
        livello città/stato. Da segnalare che 279 record cliente (158 ZIP distinti) e
        7 venditori non
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\ttfamily}l
    ...
    payment\_value       & Additiva solo a livello ordine &
      ...
      totale (nel DWH: 20,31M contro 16,01M nei pagamenti originali). Aggregare
...
\sloppy
  ...
  \item \textbf{LEFT JOIN con i pagamenti pre-aggregati}: prima del join, i
        ...
        \texttt{payment\_value} sull'intera \texttt{fact\_sales} vale 20,31M contro
        i 16,01M dei pagamenti originali aggregati per ordine.
...
\begin{itemize}
  \item \textbf{\texttt{dim\_customer}} e \textbf{\texttt{dim\_seller}} vengono
        ...
        \texttt{zip\_code\_prefix}. Il LEFT JOIN è essenziale: 279 record cliente
        (158 ZIP distinti) e 7 venditori risultano assenti in \texttt{rec\_geolocation}; un INNER
